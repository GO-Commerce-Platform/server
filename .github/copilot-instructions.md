...
-   **Testing Strategy**:
    ...
    -   **Prioritize Integration Tests**: Use `@QuarkusTest` for comprehensive testing over unit tests with mocks.
    -   **Avoid Mocking**: Do not use mocking frameworks (Mockito, etc.) unless absolutely necessary for external dependencies.
    -   **Real Data Generation**: Create real test data using UUIDs, builders, or factory methods rather than mocked objects.
    -   **Test Data Patterns**: Use unique identifiers (e.g., `UUID.randomUUID().toString().substring(0, 8)`) to ensure test isolation.
    -   **Integration Over Isolation**: Test the complete workflow including database interactions, service layers, and business logic.
    -   **Cleanup Strategy**: Implement proper cleanup in `@AfterEach` methods to maintain test isolation.
    -   **Example Pattern**:
        ...
        class ServiceImplTest {
            ...
            @Test
            @Transactional
            void testRealWorkflow() {
                // Given - Real test data
                var entityId = UUID.randomUUID();
                var testDto = new CreateEntityDto("Test Name", entityId);

                // When - Real service call
                var result = someService.createEntity(testDto);
    ...
    -   **Rationale**: Integration tests provide better confidence in the application's behavior, catch integration issues early, and align with the project's multi-tenant architecture requirements.
