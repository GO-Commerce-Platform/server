...
public class LocaleResource {
    ...
    /**
     * Comma-separated list of supported locales, injected from configuration.
     * Example: "en,pt_BR,es".
     */
    @ConfigProperty(name = "quarkus.locales")
    private Optional<String> supportedLocales;

    /**
     * Get information about current locale and some example internationalized
     * messages.
     *
    ...
    public Response getLocaleInfo() {
        ...
        messages.put("customerInfo",
                messageService.getMessage("customer.firstName") + " & "
                        + messageService.getMessage("customer.lastName"));

        // Add validation message with parameter
        messages.put("validation",
    ...
    public Response changeLocale(@PathParam("localeTag") String localeTag) {
        if (localeTag == null || localeTag.isEmpty()) {
            return Response.status(Response.Status.BAD_REQUEST)
                    .entity(Map.of("error", "Locale tag is required")).build();
        }
    ...
    private boolean isSupportedLocale(String localeTag) {
        if (supportedLocales.isEmpty()) {
            // If no supported locales are defined, we only accept known valid
            // locales
            try {
                Locale locale = Locale.forLanguageTag(localeTag);
                // Check if this is a reasonable locale (has at least a
                // language)
