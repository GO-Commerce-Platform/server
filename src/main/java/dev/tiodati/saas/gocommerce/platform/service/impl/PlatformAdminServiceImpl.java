...
public class PlatformAdminServiceImpl implements PlatformAdminService {
    ...
    public StoreResponse createStore(CreateStoreRequest request) {
        ...
        try {
            PlatformStores store = PlatformStores.builder().storeName(request.name())
    ...
    public StoreResponse updateStore(UUID storeId, UpdateStoreRequest request) {
        ...
        PlatformStores store = storeRepository.findByIdOptional(storeId)
                .orElseThrow(() -> new ResourceNotFoundException(
                        "Store not found with ID: " + storeId));

        boolean updated = false;
        if (request.name() != null && !request.name().equals(store.getStoreName())) {
