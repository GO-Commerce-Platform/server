...
public class StoreRoleAuthorizationInterceptor {
         ...
         * has the required roles and access to the current store.
         */
        private final KeycloakRoleVerificationService roleVerificationService;

        /**
         * Constructs a new StoreRoleAuthorizationInterceptor.
         *
         * @param roleVerificationService Service for verifying Keycloak roles.
         */
        @Inject
        public StoreRoleAuthorizationInterceptor(
                        KeycloakRoleVerificationService roleVerificationService) {
                this.roleVerificationService = roleVerificationService;
        }

        /**
         * Intercepts method invocations to check for required store roles. If
         * the user lacks the necessary roles or access to the current store, a
         ...
         */
        @AroundInvoke
        public Object checkStoreRoles(InvocationContext context)
                        throws Exception {
                ...
                if (annotation != null) {
                        ...
                        if (requiredRoles.length > 0) {
                                ...
                                Log.warnf("Access denied: User lacks required store role(s): %s",
                                                roleNames);
                                throw new ForbiddenException(
                                                "User lacks required store role(s): "
                                                                + roleNames);
                        }
                }
