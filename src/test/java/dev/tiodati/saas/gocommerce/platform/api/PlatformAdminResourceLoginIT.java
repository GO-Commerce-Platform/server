...
public class PlatformAdminResourceLoginIT {
     ...
     */
    private static final String PLATFORM_ADMIN_BASE_PATH = "/api/v1/platform";
    ...
    public void testCreateStoreEndpoint() {
        ...
        CreateStoreRequest request = new CreateStoreRequest("Test Store",
                "test-store", uniqueSubdomain + "@example.com", "USD", "en-US",
                null, "This is a test store");

        given().contentType(ContentType.JSON)
                .header("Authorization", "Bearer " + getPlatformAdminToken())
                .body(request).when().post(PLATFORM_ADMIN_BASE_PATH + "/stores")
    ...
    private String getPlatformAdminToken() {
        // In a real test, you would obtain a valid token, possibly by calling
        // the login endpoint
        // For now, returning a placeholder. This needs to be a valid JWT for
        // security checks to pass.
        // If Keycloak is running and configured for tests, you might
        // programmatically get a token.
        // For simplicity, if tests run without security enabled or with a mock,
        // this might be simpler.
        // This part is crucial for tests requiring authentication.
        LoginRequest loginRequest = new LoginRequest("platform-admin",
                "platform-admin"); // Corrected credentials

        String token = given().contentType(ContentType.JSON).body(loginRequest)
                .when().post("/api/v1/auth/login") // Assuming AuthResourceTest
                                                   // changes are applied
