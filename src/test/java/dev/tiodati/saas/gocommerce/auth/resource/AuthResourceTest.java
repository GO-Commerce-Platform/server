...
public class AuthResourceTest {
    ...
    void testLoginFailureInvalidCredentials() {
        ...
        given()
            ...
            .then()
            .statusCode(401)
            .body("error", is("Authentication failed: Invalid username or password"));
    }
    ...
    void testLoginFailureMissingUsername() {
        ...
        given()
            ...
            .then()
            .statusCode(400)
            .body("violations", notNullValue()); // Adjust to match actual error response for validation
    }
    ...
    void testRefreshTokenSuccess() {
        // Ensure this test runs after a successful login to have a valid refreshToken
        if (refreshToken == null) {
            System.out.println("Skipping testRefreshTokenSuccess as refreshToken is null (previous login might have failed or not run)");
    ...
    void testRefreshTokenFailureInvalidToken() {
        ...
        given()
            ...
            .then()
            .statusCode(401)
            .body("error", is("Token refresh failed: Invalid or expired refresh token"));
    }
    
    @Test
    @Order(7)
    void testValidateTokenSuccess() {
        // Ensure this test runs after a successful login/refresh to have a valid accessToken
        if (accessToken == null) {
            System.out.println("Skipping testValidateTokenSuccess as accessToken is null (previous login/refresh might have failed or not run)");
            return; // Or throw an assumption failed exception
        }
        given()
    ...
    void testValidateTokenFailureInvalidToken() {
        given()
            .queryParam("token", "invalid-access-token")
            .when()
            .get("/api/auth/validate")
            .then()
            .statusCode(401) 
    ...
    void testLogoutSuccess() {
        // Ensure this test runs after a successful login/refresh to have a valid refreshToken
        if (refreshToken == null) {
            System.out.println("Skipping testLogoutSuccess as refreshToken is null (previous login/refresh might have failed or not run)");
            return; // Or throw an assumption failed exception
        }
        given()
    ...
    void testLogoutFailureInvalidToken() {
        given()
            .queryParam("refreshToken", "completely-invalid-refresh-token-format")
            .when()
            .delete("/api/auth/logout")
            .then()
            .statusCode(500) 
